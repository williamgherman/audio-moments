...
function preload(){
  source_file = loadSound('assets/unlock.ogg'); // preload the sound
}
...
function setup() {
  ...
  */
  ...
  for (var i = 0 ; i < num_voices; i++){
     var voice = new GranularVoice(source_file,grainDur);
     // duration half a second per voice
     voices.push(voice);
...
function draw() {
    ...
    if (mouseIsPressed){
      var start_play = map(mouseX,0,width,0,src_length);
      // map mouseX to position in the source
      var pitch = map(mouseY,0,height,0.5,1.5);
      // map mouseY to the rate the sound will be played
...
GranularVoice.prototype.playGrain = function(start,rate){
  ...
  // we need to play longer than grainDur because of the rate
